...
pub fn init<'a, D: DelayMs<u8> + DelayUs<u16>>(gpiob: GPIOB, i2c1: I2C1, spi1: SPI1, ahb2: &mut AHB2, apb1r1: &mut APB1R1, apb2: &mut APB2, clocks: Clocks, delay: &'a SharedDelay<D>) -> (BmeSensor<'a, D>, SpiBus, Epd<'a, D>) {
    ...
    let epd_sck = port_b.pb3.into_alternate_push_pull(&mut port_b.moder, &mut port_b.otyper, &mut port_b.afrl);
    let epd_mosi = port_b.pb5.into_alternate_push_pull(&mut port_b.moder, &mut port_b.otyper, &mut port_b.afrl);
    let epd_miso = port_b.pb4.into_alternate_push_pull(&mut port_b.moder, &mut port_b.otyper, &mut port_b.afrl);
    ...
    let mut scl = port_b.pb8.into_alternate_open_drain(&mut port_b.moder, &mut port_b.otyper, &mut port_b.afrh);
    scl.internal_pull_up(&mut port_b.pupdr, true);

    let mut sda = port_b.pb9.into_alternate_open_drain(&mut port_b.moder, &mut port_b.otyper, &mut port_b.afrh);
...
pub fn init_uart(gpiod: GPIOD, usart2: USART2, ahb2: &mut AHB2, apb1r1: &mut APB1R1, clocks: Clocks) -> (GpsUsart, GpsEnPin) {
    ...
    let tx : TxPin = gpio.pd5.into_alternate_push_pull(&mut gpio.moder, &mut gpio.otyper, &mut gpio.afrl);
    let rx : RxPin = gpio.pd6.into_alternate_push_pull(&mut gpio.moder, &mut gpio.otyper, &mut gpio.afrl);
